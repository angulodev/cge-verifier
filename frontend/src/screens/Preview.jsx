...
import { startPayment, getReport } from '../lib/api'
  ...
  return (
    <div style={{ padding: '24px 24px 40px', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 22 }}>
        <Badge color={isOk ? C.green : C.red} bg={isOk ? C.greenBg : C.redBg}>
      ...
      <Card accent={isOk ? C.green : C.red} style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            ...
            <LCDNumber value={Math.abs(preview.differenceKwh)} unit="kWh" size="sm" color={isOk ? C.green : C.red} />
          </div>
          {!isOk && preview.differenceCLP > 0 && (
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 4 }}>Estimado en pesos</div>
              <LCDNumber value={`$${preview.differenceCLP?.toLocaleString('es-CL')}`} size="sm" color={C.red} />
            </div>
          )}
        </div>
      </Card>
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        {[
          { icon: isOk ? '✓' : '⚠️', text: isOk ? 'Cuadratura perfecta' : `Diferencia de ${Math.abs(preview.differenceKwh)} kWh entre boletas y medidor`, locked: false },
          { icon: '📈', text: 'Meses con consumo anómalo (picos estadísticos)', locked: !isPaid },
          { icon: '📋', text: 'Desglose mes a mes con análisis completo', locked: !isPaid },
        ].map((h, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '9px 0', borderBottom: i < 2 ? `1px solid ${C.border}` : 'none', opacity: h.locked ? 0.45 : 1 }}>
        ...
        {preview.missingMonths > 0 && (
          <div style={{ marginTop: 12, padding: '10px 14px', background: C.amberBg, borderRadius: 8 }}>
            ...
            <div style={{ fontSize: 11, color: C.muted }}>El análisis puede ser parcial.</div>
          </div>
        )}
      </Card>
      ...
      <Card accent={C.amber} style={{ textAlign: 'center', marginBottom: 20, padding: '20px' }}>
        {isPaid ? (
          <>
            <div style={{ fontSize: 13, color: C.green, fontWeight: 700, marginBottom: 12 }}>✓ Reporte ya disponible</div>
            <Btn fullWidth onClick={handleAction} disabled={loading}>
              {loading ? 'Cargando...' : 'Ver reporte completo →'}
            </Btn>
          </>
        ) : (
          <>
            ...
            <div style={{ fontSize: 11, color: C.muted, marginTop: 4, marginBottom: 16 }}>CLP · pago único · incluye descarga PDF</div>
            <Btn fullWidth onClick={handleAction} disabled={loading}>
              {loading ? 'Redirigiendo...' : 'Ver reporte completo →'}
            </Btn>
          </>
        )}
      </Card>

      <Btn variant="ghost" fullWidth onClick={onBack} small>← Volver</Btn>
    </div>
  )
}
